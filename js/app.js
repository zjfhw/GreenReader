...
var RSSReader, currentNavJson, deviceReady, jQT, listNavJson, mainNavJson, pullDownAction, store, subscriptionData;
...
RSSReader.FindFeed = function(query) {
  showLoader();
  if (query && query.substring(0, 6) === 'q=http') {
...
RSSReader.getSubscription = function() {
  return subscriptionData.all(function(arr) {
    arr.forEach(function(entry) {
      var item;
      item = RSSReader.Subscription.create(entry);
      return RSSReader.subscriptionController.addItem(item);
    });
    return console.log('subscription load form local:', arr.length, arr);
  });
};
...
deviceReady = function() {
  ...
  if (window.device) {
    ...
    $('.swipeToDelete').swipe(function(evt, data) {
      var $this, details;
      details = !data ? '' : data;
...
RSSReader.itemController = Em.ArrayController.create({
  ...
  markAllRead: function() {
    ...
    return RSSReader.navbarController.changeTab();
  },
  itemCount: (function() {
    return this.get('length');
  }).property('@each'),
...
RSSReader.subscriptionController = Em.ArrayController.create({
  ...
  addItem: function(item) {
    ...
    if (exists === 0) {
      ...
      this.insertAt(0, RSSReader.Subscription.create(itemToSave));
      return true;
    } else {
      return false;
    }
  ...
  itemViewClass: Em.View.extend({
    ...
    swipeEnd: function(recognizer) {
      ...
      if (Em.OneGestureDirection.Right === recognizer.swipeDirection) {
        ...
        return $this.find('small').removeClass('hide');
      } else if (Em.OneGestureDirection.Left === recognizer.swipeDirection) {
        console.log('swipt right');
        $this.find('a').removeClass('delete');
        return $this.find('small').addClass('hide');
      }
    },
    tagName: 'li',
    classNames: ['arrow'],
...
RSSReader.FooterNavBarView = Em.CollectionView.extend({
  ...
  itemViewClass: Em.View.extend({
    ...
    click: function() {
      ...
      if (this.get('content').get('action') === 'inBrowser') {
